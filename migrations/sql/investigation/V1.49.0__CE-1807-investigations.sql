...
CREATE or REPLACE TRIGGER investigation_status_history_trigger
  ...
  FOR EACH ROW EXECUTE PROCEDURE audit_history('investigation_status_code_h', 'investigation_status_code');
...
comment on column investigation.investigation_status_code.create_timestamp is 'The timestamp when the  investigation status code was created.  The timestamp is stored in UTC with no Offset.';
...
comment on column investigation.investigation_status_code.update_timestamp is 'The timestamp when the investigation status code was updated.  The timestamp is stored in UTC with no Offset.';
...
CREATE or REPLACE TRIGGER investigation_status_history_trigger
  BEFORE INSERT OR DELETE OR UPDATE ON investigation.investigation_status_code
  FOR EACH ROW EXECUTE PROCEDURE audit_history('investigation_h', 'investigation_status_code');
...
COMMENT on table investigation.investigation is 'History table for investigation table';
