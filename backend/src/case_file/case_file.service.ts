...
export class CaseFileService {
  ...
  async findOne(caseIdentifier: string) {
    ...
    const actionCodes = await this.prisma.action_type_action_xref.findMany({
      where: {
        action_type_action_xref_guid: {
          in: actionsBase.map((item) => item.action_type_action_xref_guid),
        },
      },
      select: {
        action_code: true,
    ...
    const actions: AssessmentAction[] = actionsBase.filter(action => !action.equipment).map((action) => {
    ...
    const equipmentDetails = await this.findEquipmentDetails(caseIdentifier);
  ...
  async findEquipmentDetails(caseIdentifier: string): Promise<Equipment[]> {
  ...
  async createEquipmentCase(
    createInput: CreateEquipmentInput
  ): Promise<string> {
    let caseFileGuid: string;
